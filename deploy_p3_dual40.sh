...
trap - ERR
printf '%s\n' \
  "DUAL40 DEPLOY PASS | starts=DRY | branch=$DEPLOY_BRANCH | commit=$DEPLOY_COMMIT | price=40c+40c | ladder=5->10->30 | hard_stop_after_30=true | paper_entry=relaxed_limit_no_price_regime | live_entry=balanced_stable_post_only | paper_parallel_assets=4 | live_parallel_assets=1 | paper_fill=entry_or_recorded_ask<=40c | paper_settlement=official_result_actual_fill_price | official_result_wait=true | near_touch_41=diagnostic | initial_live_arm_floor=\$35 | remaining_path_floor=35->33->29"
