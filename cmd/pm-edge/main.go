...
func main() {
	...
	isMockMode := false
	for _, arg := range args { if arg == "--mock" { isMockMode = true } }
	...
	if err != nil { util.Logger.Fatal("Database setup failed", zap.Error(err)) }
	defer db.Close()
	...
	if err := bClient.WarmupCandles(); err != nil { util.Logger.Warn("Binance warmup failed", zap.Error(err)) }
	...
	if !isMockMode { defer clClient.Stop() }

	go func() {
		util.Logger.Info("Starting REST server", zap.String("port", cfg.Port))
		if err := server.Start(cfg.Port); err != nil { util.Logger.Fatal("REST server failed", zap.Error(err)) }
	}()
	...
	go func() {
		...
		if poll < time.Second { poll = time.Second }
		ticker := time.NewTicker(poll); defer ticker.Stop()
		for {
			select {
			case <-ctx.Done(): return
			case now := <-ticker.C: refreshMarket(now.UTC())
			}
		}
	}()
	...
	go func() {
		...
		for {
			...
			case now := <-ticker.C:
				...
				if isMockMode || strings.Contains(res.DataSource, "MOCK") { continue }
				if err := db.InsertSignal(res); err != nil { util.Logger.Error("Failed to store signal in SQLite", zap.Error(err)); continue }
