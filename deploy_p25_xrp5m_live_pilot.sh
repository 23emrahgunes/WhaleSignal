...
echo "=== INSTALL LIVE SDK ==="
./.venv/bin/python -m pip install -q -r requirements-live.txt

echo "=== ARM XRP:5m ONE-CYCLE LIVE PILOT ==="
...
if [[ -z "${POLYMARKET_PRIVATE_KEY:-${PK:-}}" ]]; then
  echo "ERROR: POLYMARKET_PRIVATE_KEY/PK yok; LIVE baslatilmadi." >&2
  exit 1
fi
...
req=urllib.request.Request(url,headers={'User-Agent':'WhaleSignal-P25-XRP5m-Deploy/1.0'})
...
echo "XRP:5m LIVE PILOT ARMED | stake<=1.00 USDC | FOK | paper-trigger parity | one network cycle per arm nonce=$nonce"
