...
def _latest_scan(conn) -> dict:  # noqa: ANN001
    row = conn.execute(
        "SELECT value FROM p3_meta WHERE key=?", (LATEST_SCAN_META_KEY,)
    ).fetchone()
    if row is None:
        return {}
    try:
...
:root{--bg:#07101b;--panel:#101c2c;--line:#233650;--text:#eef5ff;--mut:#8ea5c3;--green:#20d095;--red:#f06b72;--blue:#65a9ff;--amber:#f1bd58}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:13px Inter,Arial,sans-serif}header{padding:14px 18px;border-bottom:1px solid var(--line);background:#0a1523;display:flex;gap:10px;align-items:center}h1{font-size:18px;margin:0;color:var(--blue)}.pill{padding:5px 8px;border-radius:6px;background:#17375d;font-weight:800}.wrap{max-width:1700px;margin:auto;padding:14px}.notice{border:1px solid #5f4a19;background:#251d09;padding:10px;border-radius:8px;color:#ffe1a0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;margin-top:12px}.metric,.box{background:var(--panel);border:1px solid var(--line);border-radius:9px;padding:10px}.metric b{display:block;font-size:20px}.metric span{color:var(--mut)}.boxes{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.box h2{font-size:14px;margin:0 0 8px}table{width:100%;border-collapse:collapse}th,td{padding:7px;border-bottom:1px solid #1e3048;text-align:right}th:first-child,td:first-child{text-align:left}th{color:var(--mut)}.pos{color:var(--green)}.neg{color:var(--red)}.warn{color:var(--amber)}.mono{font-family:ui-monospace,Consolas,monospace;font-size:11px}@media(max-width:900px){.boxes{grid-template-columns:1fr}}</style></head><body>
<header><h1>P3 Arbitrage Lab</h1><span class="pill">STRUCTURAL</span><span class="pill">SHADOW ONLY</span><span id="state" style="color:var(--mut)">yükleniyor…</span></header><div class="wrap"><div class="notice"><b>Model-free complete-set araştırması.</b> Fırsat=0 ise aşağıdaki Scanner Funnel neden sıfır olduğunu gösterir. Private key, signing veya gerçek emir yoktur.</div><div class="grid" id="metrics"></div><div class="box" style="margin-top:12px"><h2>Scanner Funnel / Book Transport</h2><div class="grid" id="scanner"></div></div><div class="boxes"><div class="box"><h2>Stratejiler</h2><table><thead><tr><th>Strateji</th><th>Fırsat</th><th>Peak PnL</th><th>Avg ROI</th></tr></thead><tbody id="strategies"></tbody></table></div><div class="box"><h2>İki-Bacak Replay</h2><table><thead><tr><th>Delay</th><th>N</th><th>Pair Fill</th><th>One Leg</th><th>Avg PnL</th></tr></thead><tbody id="replays"></tbody></table></div></div><div class="box" style="margin-top:12px"><h2>Son Fırsatlar</h2><div style="overflow:auto;max-height:520px"><table><thead><tr><th>Combo</th><th>Strateji</th><th>q</th><th>Net PnL</th><th>ROI</th><th>Last-change Skew</th><th>Last-change Age</th></tr></thead><tbody id="recent"></tbody></table></div></div></div><script>
const $=id=>document.getElementById(id);const n=(v,d=4)=>v==null?'—':Number(v).toFixed(d);const pc=v=>v==null?'—':(Number(v)*100).toFixed(2)+'%';const m=(v,l,k='')=>`<div class="metric"><b class="${k}">${v??0}</b><span>${l}</span></div>`;async function tick(){try{const r=await fetch('/api/summary',{cache:'no-store'});const d=await r.json();const s=d.scanner||{},t=s.book_transport||{};$('state').textContent='OK · '+new Date().toLocaleTimeString();$('metrics').innerHTML=[['opportunities','Fırsat'],['windows','Window'],['replays','Replay']].map(([k,l])=>m(d[k]??0,l)).join('')+m(n(d.lifetime?.median_ms,0)+' ms','Median lifetime')+m(n(d.lifetime?.p90_ms,0)+' ms','P90 lifetime');$('scanner').innerHTML=m(s.conditions??0,'Aktif condition')+m(s.valid_pairs??0,'Geçerli book pair',s.valid_pairs?'pos':'warn')+m(s.missing_book??0,'Missing book',s.missing_book?'neg':'')+m(s.transport_stale??0,'Transport stale',s.transport_stale?'neg':'')+m(s.session_incomplete??0,'Session incomplete',s.session_incomplete?'warn':'')+m(s.missing_fee??0,'Missing fee',s.missing_fee?'neg':'')+m(s.high_source_skew??0,'Yüksek last-change skew',s.high_source_skew?'warn':'')+m(s.positive_buy_merge??0,'BUY+MERGE pozitif','pos')+m(s.positive_split_sell??0,'SPLIT+SELL pozitif','pos')+m(s.inserted??0,'Kaydedilen fırsat','pos')+m(t.connected?'LIVE':'DOWN','Book socket',t.connected?'pos':'neg')+m(t.subscribed_tokens??0,'Subscribed token')+m(t.active_conditions??0,'Collector aktif condition')+m(t.collected_conditions??0,'Prefetch dahil condition');$('strategies').innerHTML=Object.entries(d.strategies||{}).map(([k,v])=>`<tr><td class="mono">${k}</td><td>${v.opportunities}</td><td class="${v.peak_profit_usdc>=0?'pos':'neg'}">$${n(v.peak_profit_usdc)}</td><td>${pc(v.avg_roi)}</td></tr>`).join('');$('replays').innerHTML=Object.entries(d.replay_by_delay||{}).map(([k,v])=>`<tr><td>${k}ms</td><td>${v.n}</td><td>${pc(v.pair_completion_rate)}</td><td>${v.one_leg}</td><td>$${n(v.avg_cycle_pnl_usdc)}</td></tr>`).join('');$('recent').innerHTML=(d.recent||[]).map(v=>`<tr><td>${v.combo_key}</td><td class="mono">${v.strategy}</td><td>${n(v.quantity_shares,2)}</td><td class="${v.net_profit_usdc>=0?'pos':'neg'}">$${n(v.net_profit_usdc)}</td><td>${pc(v.net_roi)}</td><td>${v.source_skew_ms}ms</td><td>${v.max_book_age_ms}ms</td></tr>`).join('')}catch(e){$('state').textContent='HATA '+e}}setInterval(tick,3000);tick();</script></body></html>"""
