...
func TestDatabaseMigrationInsertAndRead(t *testing.T) {
	...
	db, err := NewDatabase(path); if err != nil { t.Fatalf("NewDatabase: %v", err) }
	defer db.Close()
	if err := db.InsertSignal(validSignal()); err != nil { t.Fatalf("InsertSignal: %v", err) }
	history, err := db.GetHistory(10); if err != nil { t.Fatalf("GetHistory: %v", err) }
	if len(history)!=1 { t.Fatalf("history length got %d want 1",len(history)) }
	if history[0].Slug!="btc-updown-5m-1770000000" || history[0].DataSource!="CHAINLINK_RTDS+BINANCE_WS" { t.Fatalf("unexpected history row: %#v",history[0]) }
}

func TestInsertSignalRejectsInvalidRows(t *testing.T) {
	path:=filepath.Join(t.TempDir(),"signals.sqlite")
	db,err:=NewDatabase(path);if err!=nil{t.Fatal(err)};defer db.Close()
	cases:=[]struct{name string; mutate func(*engine.EvaluationResult)}{
	...
	for _,tc:=range cases{t.Run(tc.name,func(t *testing.T){r:=validSignal();tc.mutate(r);if err:=db.InsertSignal(r);err==nil{t.Fatalf("expected rejection")}})}
}

func TestMigrationPurgesExactLegacySyntheticFallback(t *testing.T) {
	path:=filepath.Join(t.TempDir(),"signals.sqlite")
	db,err:=NewDatabase(path);if err!=nil{t.Fatal(err)}
	if err:=db.InsertSignal(validSignal());err!=nil{t.Fatal(err)}
	if _,err:=db.db.Exec(`UPDATE signals SET slug='btc-above-100k-1505', price_to_beat=100000`);err!=nil{t.Fatal(err)}
	if err:=db.Close();err!=nil{t.Fatal(err)}

	db2,err:=NewDatabase(path);if err!=nil{t.Fatal(err)};defer db2.Close()
	history,err:=db2.GetHistory(10);if err!=nil{t.Fatal(err)}
	if len(history)!=0{t.Fatalf("legacy synthetic row survived migration: %#v",history)}
}
