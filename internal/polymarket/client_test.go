...
func TestFetchActiveBTC5mMarketAt(t *testing.T) {
	...
	if err != nil { t.Fatalf("FetchActiveBTC5mMarketAt: %v", err) }
	if requestedPath != "/events/slug/btc-updown-5m-1770000000" { t.Fatalf("unexpected path %q", requestedPath) }
	if !m.StartTime.Equal(start) || !m.EndTime.Equal(start.Add(5*time.Minute)) { t.Fatalf("bad market window: %s - %s", m.StartTime, m.EndTime) }
	if len(m.Tokens) != 2 || m.Tokens[0].TokenID != "up-token" || m.Tokens[0].Price != 0.52 { t.Fatalf("unexpected token parsing: %#v", m.Tokens) }
	if m.PriceToBeat != 0 { t.Fatalf("market discovery must not invent PTB; got %.2f", m.PriceToBeat) }
}

func TestFetchPriceToBeat(t *testing.T) {
	start := time.Unix(1770000000, 0).UTC()
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/crypto/crypto-price" { http.NotFound(w, r); return }
		if r.URL.Query().Get("symbol") != "BTC" || r.URL.Query().Get("variant") != "fiveminute" { t.Errorf("unexpected query: %s", r.URL.RawQuery) }
		if !strings.Contains(r.URL.Query().Get("eventStartTime"), "T") { t.Errorf("eventStartTime missing RFC3339: %s", r.URL.RawQuery) }
		w.Header().Set("Content-Type", "application/json")
	...
	if err != nil { t.Fatalf("FetchPriceToBeat: %v", err) }
	if got != 64000.25 { t.Fatalf("openPrice: got %.2f", got) }
}
