...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        ...
        case_sensitive=False, env_ignore_empty=True,
    )
    ...
    max_reference_open_alignment_ms: float = Field(default=5000.0, alias="MAX_REFERENCE_OPEN_ALIGNMENT_MS")
    max_reference_source_age_ms: float = Field(default=8000.0, alias="MAX_REFERENCE_SOURCE_AGE_MS")
    ...
    binance_ws_base: str = Field(default="wss://stream.binance.com:9443/stream", alias="BINANCE_WS_BASE")
    ...
    binance_book_snapshot_limit: int = Field(default=1000, alias="BINANCE_BOOK_SNAPSHOT_LIMIT")
    ring_buffer_max: int = Field(default=4000, alias="RING_BUFFER_MAX")
    feature_price_ring_max: int = Field(default=24000, alias="FEATURE_PRICE_RING_MAX")
    ...
    clob_ws_url: str = Field(default="wss://ws-subscriptions-clob.polymarket.com/ws/market", alias="CLOB_WS_URL")
    ...
    snapshot_checkpoints_csv: str = Field(default="240,180,120,90,60,45,30,20,10,5", alias="SNAPSHOT_CHECKPOINTS")
    ...
    checkpoints_5m_csv: str = Field(default="240,180,150,120,90,60,45,30,20,10", alias="CHECKPOINTS_5M")
    checkpoints_15m_csv: str = Field(default="840,720,600,480,360,240,120,60,30", alias="CHECKPOINTS_15M")
    checkpoints_1h_csv: str = Field(default="3300,3000,2400,1800,1200,900,600,300,120,60", alias="CHECKPOINTS_1H")
    ...
    def snapshot_checkpoints(self) -> list[int]:
        out = [int(t.strip()) for t in self.snapshot_checkpoints_csv.split(",") if t.strip().isdigit()]
