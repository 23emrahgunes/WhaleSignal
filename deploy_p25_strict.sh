...
wanted = {
    ...
    # Retained only for backwards-compatible config loading. The ALL5m executor no
    # longer anchors its live price to paper_fill * (1+drift).
...
print('live_pre_submit_book_check=', live.get('pre_submit_book_check'))
print('live_matching_engine_liquidity_gate=', live.get('matching_engine_is_liquidity_gate'))
...
assert live.get('execution_price_mode') == 'SIGNAL_IMMEDIATE_LIMIT_CAP'
...
assert live.get('matching_engine_is_liquidity_gate') is True
